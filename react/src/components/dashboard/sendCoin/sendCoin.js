    ...
    if (_mode === 'native') {
      if (_coin === 'SAFE') { // reject t -> z
        if ((!this.state.sendFrom || (this.state.sendFrom && this.state.sendFrom.substring(0, 2) !== 'zc' && this.state.sendFrom.substring(0, 2) !== 'zs' && this.state.sendFrom.length <= 78)) &&
          ...
          Store.dispatch(
            triggerToaster(
              'Private transaction are being disabled on 15th February 2019 on SAFE chain permanently and will become a public chain where only transparent transactions are possible. Please don\'t send them to private address which can lead to SAFE being locked after the deadline 15th February 2019.',
              translate('TOASTR.WALLET_NOTIFICATION'),
              'warning toastr-wide'
            )
          );
